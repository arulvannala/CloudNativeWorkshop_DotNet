...
. Open the _FortuneTeller.sln_ solution as the starting point and begin adding code for this lab and each of the remaining labs. With this path, you will be writing new code and debugging what you write.

If you start with the _FortuneTeller.sln_, the app in its current state is not fully functional:

. The ``Fortune Teller Service`` only serves up a single Fortune.
. The ``Fortune Teller UI`` doesn't know how to communicate with the ``Fortune Teller Service`` so it always services up a ``Hello world`` Fortune.

The goals for Lab 6 are to understand ASP.NET Core dependency injection and use it to wire up two areas of the code:

. Fortune Teller Service:
.. Wire up the ``IFortuneRepository`` to the ``FortuneController`` and modify the ``FortuneController`` to use the ``FortuneRepository``
.. Add the ``IFortuneRepository`` and the ``FortuneContext`` to the service container.
.. Initialize the ``FortuneContext`` with some Fortunes.
. Fortune Teller UI:
.. Wire up the ``IFortuneService`` to the ``FortuneController`` and modify the ``FortuneController`` to use the ``IFortuneService``.
.. Add the ``IFortuneService`` to the service container.

At the end of this lab, you will have an app that:

. Has a ``Fortune Teller Service``  that uses a backend in-memory datastore to hold multiple ``Fortunes``.
. Has a ``Fortune Teller Service`` that no longer serves up a single Fortune, but serves up random fortunes from the datastore.
. Has a ``Fortune Teller UI`` that uses a stubbed out ``IFortuneService``.
--
